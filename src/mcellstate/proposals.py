...
class BlockPayload:
    ...
    @classmethod
    def from_cells(
        cls, state: PartitionState, cells: tuple[int, ...]
    ) -> "BlockPayload":
...
Proposal = (
    MergeProposal | PeelProposal | MoveProposal | BlockPeelProposal | BlockMoveProposal
)
...
class ProposalSampler:
    def __init__(
        ...
        *,
        ...
        self.merge_uniform_prob = float(
            epsilon_uniform if merge_uniform_prob is None else merge_uniform_prob
        )
        self.peel_uniform_prob = float(
            epsilon_uniform if peel_uniform_prob is None else peel_uniform_prob
        )
        self.move_uniform_prob = float(
            epsilon_uniform if move_uniform_prob is None else move_uniform_prob
        )
        self.block_peel_uniform_prob = float(
            epsilon_uniform
            if block_peel_uniform_prob is None
            else block_peel_uniform_prob,
        )
        self.block_move_uniform_prob = float(
            epsilon_uniform
            if block_move_uniform_prob is None
            else block_move_uniform_prob,
        )
        ...
        self.random_proposals = bool(random_proposals)
        self.max_unique_proposals = (
            None
            if max_unique_proposals is None or int(max_unique_proposals) <= 0
            else int(max_unique_proposals)
        )
    ...
    def set_family_weights(self, **weights: float) -> None:
        updated = []
        for family_name, old_weight in zip(
            self.family_names, self.family_weights, strict=True
        ):
        ...
        if np.any(array < 0) or array.sum() <= 0.0:
            raise ValueError(
                "family weights must stay non-negative with positive total mass"
            )
    ...
    def notify_state_changed(
        ...
        self._pending_signature_refresh.update(
            int(cluster_id) for cluster_id in touched_clusters
        )
        self._pending_gene_refresh.update(
    ...
    def prepare_round(self, state: PartitionState) -> None:
        ...
        self._non_singletons = np.asarray(
            sorted(state.non_singleton_cluster_ids()), dtype=np.int64
        )
        if self._non_singletons.size:
            sizes = np.asarray(
                [state.cluster_size(cluster_id) for cluster_id in self._non_singletons],
                dtype=np.float64,
            )
            total = float(sizes.sum())
            self._non_singleton_probs = (
                sizes / total
                if total > 0.0
                else np.full_like(sizes, 1.0 / float(len(sizes)))
            )
        else:
            self._non_singleton_probs = np.empty(0, dtype=np.float64)
        ...
        self._idf_weights = np.log(
            (len(self._active_clusters) + 1.0)
            / (state.gene_cluster_counts.astype(np.float64) + 1.0),
        )
        ...
        if self.random_proposals:
            ...
            self._emit_trace(
                ...
                "bad_cell_cache_s=0.000 "
                "signature_pool_s=0.000 "
                "signature_current_s=0.000 "
                "merge_neighbors_s=0.000 "
                "random_proposals=1 "
    ...
    def _refresh_bad_cell_cache(self, state: PartitionState) -> None:
        active_non_singletons = set(
            int(cluster_id) for cluster_id in self._non_singletons.tolist()
        )
        stale_clusters = set(self._bad_cell_candidates).difference(
            active_non_singletons
        )
        ...
        if not self._bad_cell_candidates or self._prepared_version is None:
            ...
            refresh_clusters.update(
                active_non_singletons.difference(self._bad_cell_candidates)
            )
        refresh_list = sorted(refresh_clusters)
        if not refresh_list:
            return
        refresh_seeds = self.rng.integers(
            np.iinfo(np.int64).max, size=len(refresh_list), dtype=np.int64
        )
        ...
        def build_bad_cells(task: tuple[int, int]) -> tuple[int, np.ndarray]:
            ...
            poor_fit_scores = np.asarray(
                [
                    self._cell_poor_fit_score(state, int(cluster_id), int(cell))
                    for cell in sampled
                ],
                dtype=np.float64,
            )
            order = np.argsort(poor_fit_scores)[::-1][
                : min(self.low_fit_cache_size, sampled.size)
            ]
    ...
    def _refresh_signature_pools(self, state: PartitionState) -> None:
        ...
        if not self._signature_pools:
            ...
            refresh_clusters = {
                ...
                if int(cluster_id) not in self._signature_pools
            }
            refresh_clusters.update(
                int(cluster_id)
                for cluster_id in self._pending_signature_refresh
    ...
    def _refresh_current_signatures(self, state: PartitionState) -> None:
        ...
        def build_signature(
            ...
            pool_counts = (
                state.clusters[int(cluster_id)].get_many(pool).astype(np.float64)
            )
        ...
        for cluster_id, chosen_genes, chosen_weights in self._parallel_map(
            active_clusters, build_signature
        ):
    ...
    def _refresh_cluster_lookup(self) -> None:
        ...
        self._cluster_lookup[self._active_clusters] = np.arange(
            self._active_clusters.size, dtype=np.int64
        )
    ...
    def _candidate_cluster_data_for_gene(
        ...
        if cached is not None:
            ...
            cluster_ids = np.fromiter(
                (int(cluster_id) for cluster_id in clusters),
                dtype=np.int64,
                count=len(clusters),
            )
            counts = np.asarray(
                [
                    state.clusters[int(cluster_id)].get(int(gene))
            ...
            if cluster_ids.size > self.merge_gene_cluster_cap:
                cluster_ids = np.asarray(
                    cluster_ids[: self.merge_gene_cluster_cap], dtype=np.int64
                )
                counts = np.asarray(
                    counts[: self.merge_gene_cluster_cap], dtype=np.int64
                )
    ...
    def _candidate_clusters_for_gene(
        self, state: PartitionState, gene: int
    ) -> np.ndarray:
    ...
    def _top_cluster_ids_from_scores(
        ...
        *,
        ...
        if (
            ...
            and 0 <= int(exclude_cluster) < self._cluster_lookup.size
        ):
            source_pos = int(self._cluster_lookup[int(exclude_cluster)])
            if source_pos >= 0:
                candidate_positions = candidate_positions[
                    candidate_positions != source_pos
                ]
        if candidate_positions.size == 0:
            return []
        if limit is not None and candidate_positions.size > int(limit):
            top_rel = np.argpartition(scores[candidate_positions], -int(limit))[
                -int(limit) :
            ]
    ...
    def _build_merge_neighbors_for_source(
        ...
        if (
            signature_genes is None
            or signature_weights is None
            or signature_genes.size == 0
        ):
            return np.empty(0, dtype=np.int64), np.empty(0, dtype=np.float64), []
        scores = np.zeros(self._active_clusters.size, dtype=np.float64)
        source_pos = (
            int(self._cluster_lookup[int(source_cluster)])
            if int(source_cluster) < self._cluster_lookup.size
            else -1
        )
        for gene, source_weight in zip(signature_genes, signature_weights, strict=True):
            ...
            if idf <= 0.0:
                continue
            _, positions, counts = self._candidate_cluster_data_for_gene(
                state, int(gene)
            )
            if positions.size == 0:
                continue
        ...
        ranked_ids = self._top_cluster_ids_from_scores(
            scores, limit=self.top_merge_neighbors, exclude_cluster=source_cluster
        )
        if not ranked_ids:
            return np.empty(0, dtype=np.int64), np.empty(0, dtype=np.float64), []
        ...
        ranked = [
            ...
            for cluster_id, score in zip(neighbors, neighbor_scores, strict=True)
        ]
    ...
    def _remove_merge_source(self, source_cluster: int) -> None:
        ...
        stale_pairs = [
            pair for pair in self._merge_pair_scores if source_cluster in pair
        ]
        for pair in stale_pairs:
            del self._merge_pair_scores[pair]

    def _prune_inactive_merge_targets(self, active_set: set[int]) -> None:
        for source_cluster in list(self._merge_neighbors):
            if int(source_cluster) not in active_set:
                self._remove_merge_source(int(source_cluster))
                continue
            neighbors = self._merge_neighbors[int(source_cluster)]
            weights = self._merge_neighbor_weights.get(int(source_cluster))
            if weights is None or weights.size != neighbors.size:
                self._remove_merge_source(int(source_cluster))
                continue
            active_mask = np.fromiter(
                (
                    int(neighbor) in active_set and int(neighbor) != int(source_cluster)
            ...
            if not np.any(active_mask):
                self._remove_merge_source(int(source_cluster))
                continue
            pruned_neighbors = neighbors[active_mask]
            pruned_weights = weights[active_mask]
            weight_sum = float(pruned_weights.sum())
            if weight_sum <= 0.0:
                self._remove_merge_source(int(source_cluster))
                continue
            self._merge_neighbors[int(source_cluster)] = np.asarray(
                pruned_neighbors, dtype=np.int64
            )
            self._merge_neighbor_weights[int(source_cluster)] = np.asarray(
                pruned_weights / weight_sum, dtype=np.float64
            )

        self._merge_pair_scores = {
            pair: score
            for pair, score in self._merge_pair_scores.items()
            if int(pair[0]) in active_set
            and int(pair[1]) in active_set
            and int(pair[0]) != int(pair[1])
        }

    def _rebuild_merge_neighbors(
        self, state: PartitionState, *, full_rebuild: bool
    ) -> None:
        active_set = set(
            int(cluster_id) for cluster_id in self._active_clusters.tolist()
        )
        ...
        if full_rebuild:
            ...
            self._prune_inactive_merge_targets(active_set)
            impacted_sources = set(
                int(cluster_id)
                for cluster_id in self._pending_signature_refresh
                if int(cluster_id) in active_set
            )
            ...
            for gene in self._pending_gene_refresh:
                impacted_sources.update(
                    int(cluster_id)
                    for cluster_id in self._signature_gene_sources.get(int(gene), ())
                )
                cluster_ids, _, _ = self._candidate_cluster_data_for_gene(
                    state, int(gene)
                )
                impacted_sources.update(
                    int(cluster_id) for cluster_id in cluster_ids.tolist()
                )
        ...
        def build_source(
            cluster_id: int,
        ) -> tuple[int, np.ndarray, np.ndarray, list[tuple[int, float]]]:
            ...
            neighbors, weights, ranked = self._build_merge_neighbors_for_source(
                state, int(cluster_id)
            )
            return int(cluster_id), neighbors, weights, ranked

        for cluster_id, neighbors, weights, ranked in self._parallel_map(
            rebuild_list, build_source
        ):
            if neighbors.size == 0:
                continue
            ...
            for target_cluster, score in ranked:
                ...
                self._merge_pair_scores[pair] = self._merge_pair_scores.get(
                    pair, 0.0
                ) + float(score)

        ordered_pairs = sorted(
            self._merge_pair_scores.items(), key=lambda item: item[1], reverse=True
        )
    ...
    def sample_batch(self, state: PartitionState, n_proposals: int) -> list[Proposal]:
        ...
        active_set = set(int(cluster_id) for cluster_id in state.active_cluster_ids)
        proposals.extend(
            self._deterministic_merge_proposals(
        ...
        self._emit_trace(
            ...
            f"requested={requested_proposals} draw={draw_count} "
        ...
        if self.proposal_workers > 1 and len(sub_tasks) > 1:
            seeds = self.rng.integers(
                np.iinfo(np.int64).max, size=len(sub_tasks), dtype=np.int64
            )
        ...
        if len(proposals) != before_dedupe:
            self._emit_trace(
                f"sampler deduplicated proposals before={before_dedupe} after={len(proposals)}"
            )
        before_cap = len(proposals)
        proposals = self._cap_unique_proposals(proposals)
        if len(proposals) != before_cap:
            self._emit_trace(
                f"sampler capped proposals before={before_cap} after={len(proposals)}"
            )
        self._emit_trace(
    ...
    def _proposal_draw_count(self, requested_proposals: int) -> int:
        requested_proposals = int(requested_proposals)
        if (
            not self.random_proposals
            or self.max_unique_proposals is None
            or requested_proposals <= self.max_unique_proposals
        ):
            return requested_proposals
        oversampled_cap = int(np.ceil(float(self.max_unique_proposals) * 1.25))
        return max(1, min(requested_proposals, oversampled_cap))

    def _expand_family_tasks(
        self, family_tasks: list[tuple[str, int]]
    ) -> list[tuple[str, int]]:
        ...
        for family_name, count in family_tasks:
            if count <= 0:
                continue
            if (
                family_name not in {"move", "block_move"}
                or self.proposal_workers <= 1
                or count < max(32, self.proposal_workers * 4)
            ):
    ...
    def _deterministic_merge_proposals(
        self, budget: int, *, active_set: set[int]
    ) -> list[MergeProposal]:
        return [
            ...
            if int(cluster_a) in active_set and int(cluster_b) in active_set
        ]

    def _deduplicate_proposals(
        self, proposals: list[Proposal], *, active_set: set[int]
    ) -> list[Proposal]:
        unique: dict[tuple, Proposal] = {}
        for proposal in proposals:
            key = self._proposal_key(proposal, active_set=active_set)
            if key is None:
                continue
            unique.setdefault(key, proposal)
        return list(unique.values())

    def _cap_unique_proposals(self, proposals: list[Proposal]) -> list[Proposal]:
        if (
            self.max_unique_proposals is None
            or len(proposals) <= self.max_unique_proposals
        ):
            return proposals
        cap = int(self.max_unique_proposals)
        if cap <= 0:
            return []
        if self.random_proposals:
            selected = self.rng.choice(len(proposals), size=cap, replace=False)
            return [proposals[int(idx)] for idx in selected.tolist()]
        return proposals[:cap]

    def _proposal_key(
        self, proposal: Proposal, *, active_set: set[int]
    ) -> tuple | None:
        if isinstance(proposal, MergeProposal):
            if (
                proposal.cluster_a not in active_set
                or proposal.cluster_b not in active_set
            ):
                return None
            return (
                "merge",
                min(proposal.cluster_a, proposal.cluster_b),
                max(proposal.cluster_a, proposal.cluster_b),
            )
        if isinstance(proposal, PeelProposal):
            if proposal.source_cluster not in active_set:
                return None
            return ("peel", proposal.cell, proposal.source_cluster)
        if isinstance(proposal, MoveProposal):
            if (
                proposal.source_cluster not in active_set
                or proposal.target_cluster not in active_set
            ):
                return None
            return (
                "move",
    ...
    def _sample_family(
        self, state: PartitionState, family: str, count: int
    ) -> list[Proposal]:
        ...
        for _ in range(count):
            ...
            if self.random_proposals:
                if family == "merge":
                    ...
                    proposal = self._sample_peel_uniform(state)
                elif family == "move":
                    proposal = self._sample_move_uniform(state)
                elif family == "block_peel":
                    proposal = self._sample_block_peel_uniform(state)
                ...
                if family == "merge":
                    proposal = (
                        self._sample_merge_uniform(state)
                        if use_uniform
                        else self._sample_merge_biased(state)
                    )
                else:
                    proposal = (
                        self._sample_peel_uniform(state)
                        if use_uniform
                        else self._sample_peel_biased(state)
                    )
            if proposal is not None:
                sampled.append(proposal)
        return sampled

    def merge_neighbors_for(
        self, cluster_id: int, limit: int | None = None
    ) -> np.ndarray:
        neighbors = self._merge_neighbors.get(
            int(cluster_id), np.empty(0, dtype=np.int64)
        )
        if limit is None:
            return neighbors
    ...
    def rank_target_clusters(
        ...
        *,
        ...
        if (
            ...
            and np.asarray(candidate_targets, dtype=np.int64).size
        ):
            ...
            candidate_positions = (
                self._cluster_lookup[candidate_targets]
        ...
        for gene, value in zip(
            ...
            scores[gene_positions] += (
                np.minimum(float(value), counts[valid_positions].astype(np.float64))
                * idf
            )
        ranked_ids = self._top_cluster_ids_from_scores(
            scores,
            limit=limit or self.move_neighbor_limit,
            exclude_cluster=source_cluster,
        )
        if not ranked_ids:
            neighbors = self.merge_neighbors_for(
                int(source_cluster), limit or self.move_neighbor_limit
            )
    ...
    def _sample_merge_uniform(self, state: PartitionState) -> MergeProposal | None:
        del state
        n_active = int(self._active_clusters.size)
        if n_active < 2:
            return None
        first_pos = int(self.rng.integers(n_active))
        second_pos = int(self.rng.integers(n_active - 1))
        if second_pos >= first_pos:
            second_pos += 1
        cluster_a, cluster_b = sorted(
            (
                int(self._active_clusters[first_pos]),
                int(self._active_clusters[second_pos]),
            )
        )
    ...
    def _sample_non_singleton_cell(
    ...
    def _sample_low_fit_cell(self, state: PartitionState, source_cluster: int) -> int:
        ...
        if cached is not None and cached.size:
            if cached.size == 1:
                return int(cached[0])
            weights = np.linspace(
                float(cached.size), 1.0, int(cached.size), dtype=np.float64
            )
            weights = weights / weights.sum()
        ...
        poor_fit_scores = np.asarray(
            [
                self._cell_poor_fit_score(state, source_cluster, int(cell))
                for cell in sampled
            ],
            dtype=np.float64,
        )
    ...
    def _sample_move_uniform(self, state: PartitionState) -> MoveProposal | None:
        ...
        if target_cluster is None:
            return None
        return MoveProposal(
            cell=cell, source_cluster=source_cluster, target_cluster=target_cluster
        )
    ...
    def _sample_block_move_uniform(
        self, state: PartitionState
    ) -> BlockMoveProposal | None:
        sampled = self._sample_block_source(state, biased=False)
        if sampled is None:
            return None
        source_cluster, block = sampled
        ...
        if target_cluster is None:
            return None
        return BlockMoveProposal(
            block=block, source_cluster=source_cluster, target_cluster=target_cluster
        )

    def _sample_block_move_biased(
        self, state: PartitionState
    ) -> BlockMoveProposal | None:
        sampled = self._sample_block_source(state, biased=True)
        if sampled is None:
        ...
        source_cluster, block = sampled
        candidate_targets = self._candidate_targets_for_payload(
            state, source_cluster, block.indices, block.values
        )
        targets = self.rank_target_clusters(
            state,
            source_cluster,
        ...
        target_cluster = int(
            targets[0]
            if len(targets) == 1
            else self.rng.choice(np.asarray(targets, dtype=np.int64))
        )
        return BlockMoveProposal(
            block=block, source_cluster=source_cluster, target_cluster=target_cluster
        )
    ...
    def _candidate_targets_for_payload(
        ...
        merge_neighbors = self.merge_neighbors_for(
            int(source_cluster), self.move_neighbor_limit
        )
        if merge_neighbors.size:
            candidates.append(np.asarray(merge_neighbors, dtype=np.int64))
        if genes.size:
            scores = values.astype(np.float64) * np.maximum(
                self._idf_weights[np.asarray(genes, dtype=np.int64)], 0.0
            )
            ...
            for gene in np.asarray(genes, dtype=np.int64)[focus_order]:
                ...
                if gene_candidates.size:
                    candidates.append(
                        np.asarray(
                            gene_candidates[: self.move_candidate_cap_per_gene],
                            dtype=np.int64,
                        )
                    )
        if not candidates:
        ...
        pooled = np.fromiter(
            (
                ...
                for cluster_id in dict.fromkeys(
                    int(cluster_id) for cluster_id in ordered.tolist()
                )
                if cluster_id != int(source_cluster)
            ),
            dtype=np.int64,
        )
        if pooled.size == 0:
    ...
    def _sample_block_source(
        ...
        *,
        ...
        if biased:
            ...
            cells = np.asarray(
                state.cells_by_cluster[source_cluster].cells, dtype=np.int64
            )
            selected = tuple(
                sorted(
                    int(cell)
                    for cell in self.rng.choice(cells, size=target_size, replace=False)
                )
            )
    ...
    def _biased_block_payload(
        ...
        similarity_scores = np.asarray(
            [
                self._shared_rare_overlap(state, seed_genes, seed_values, int(cell))
                for cell in sampled
            ],
            dtype=np.float64,
        )
    ...
    def _sample_uniform_target(self, source_cluster: int) -> int | None:
        ...
        if n_active < 2:
            return None
        source_cluster = int(source_cluster)
        if source_cluster >= self._cluster_lookup.size:
            return int(self._active_clusters[int(self.rng.integers(n_active))])
        source_pos = int(self._cluster_lookup[source_cluster])
        if source_pos < 0:
            return int(self._active_clusters[int(self.rng.integers(n_active))])
        target_pos = int(self.rng.integers(n_active - 1))
        if target_pos >= source_pos:
            target_pos += 1
        if target_pos < 0 or target_pos >= n_active:
            return None
        return int(self._active_clusters[target_pos])

    def _cell_fit_score(
        self, state: PartitionState, source_cluster: int, cell: int
    ) -> float:
    ...
    def _rank01(scores: np.ndarray) -> np.ndarray:
        ...
        ranks = np.argsort(
            np.argsort(scores, kind="mergesort"), kind="mergesort"
        ).astype(np.float64)
    ...
    def _shared_rare_overlap(
        ...
        seed_map = {
            int(gene): int(value)
            for gene, value in zip(seed_genes, seed_values, strict=True)
        }
        score = 0.0
        for gene, value in zip(genes, values, strict=True):
            ...
            if gene not in seed_map:
                continue
            score += min(float(value), float(seed_map[gene])) * max(
                float(self._idf_weights[gene]), 0.0
            )
        return float(score)
